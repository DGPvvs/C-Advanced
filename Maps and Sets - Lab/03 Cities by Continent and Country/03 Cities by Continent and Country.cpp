...
int main()
{
	...
	std::map<std::string, std::map<std::string, std::set<std::string>>> continents = std::map<std::string, std::map<std::string, std::set<std::string>>>();
	...
	for (size_t i = 0; i < n; i++)
	{
		...
		std::string continent = inputs[0], country = inputs[1], town = inputs[2];
		
		if (!continents.count(continent))
		{
			continents[continent] = std::map<std::string, std::set<std::string>>();
		}

		if (!continents[continent].count(country))
		{
			continents[continent][country] = std::set<std::string>();
		}

		continents[continent][country].insert(town);
	}
	std::string output = std::string();

	for (auto itContinents = continents.begin(); itContinents != continents.end(); itContinents++)
	{
		output.append(itContinents->first).append(":").append(myString().NewLine());

		auto c = itContinents->second;

		for (auto itCountri = c.begin(); itCountri != c.end(); itCountri++)
		{
			auto v = itCountri->second;
			output
				.append("  ")
				.append(itCountri->first)
				.append(" -> ")
				.append(input.Join(", ", &v))
...
//Write a program that reads continents, countries, and their cities, puts them in a nested Map and prints them.