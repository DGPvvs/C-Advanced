// 01 Sum Matrix Columns.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
    ...
    for (int j = 0; j < m; j++)
    {
        ...
        output.append(std::to_string(sum)).append(myString().NewLine());
    }
...
//Write a program that read a matrix from the console and prints the sum for each column.On the first line, you will get matrix rows.On the next rows lines, you will get elements for each column separated with a space.