// 1 TryParse.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
	...
	int* num1 = &num11;
	int* num2 = &num12;
	myString s1 = myString(list[0]);
	myString s2 = myString(list[1]);

	bool isNum1 = s1.TryParseInt(*num1);
	bool isNum2 = s2.TryParseInt(*num2);
	...
	{
		int sum = *num1 + *num2;
		output.append(std::to_string(sum));
	}
	else if (isNum1 && !isNum2)
	{
		output
			.append(std::to_string(*num1))
			.append(myString().NewLine())
			.append(ERROR).append(s2);
	}
	else if (!isNum1 && isNum2)
	{
		output
			.append(ERROR).append(s1)
			.append(myString().NewLine())
			.append(std::to_string(*num2));
	}
	else
	{
		output
