// 02 Basic Queue Operations.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
	...
	std::queue<int> commandsQu = input.ConvertStringToIntQueue();

	std::getline(std::cin, input);
	std::queue<int> listQu = input.ConvertStringToIntQueue();

	commandsQu.pop();

	for (int i = 0; i < commandsQu.front(); i++)
	{
		listQu.pop();
	}
	...
	while (listQu.size() > 0 && !isLoopExit)
	{
		int currentNum = listQu.front();
		...
		{
			minimal = std::min(minimal, currentNum);
		}
...
//You will be given an integer N representing the number of elements to add,
//an integer S representing the number of elements to remove from the queue,
