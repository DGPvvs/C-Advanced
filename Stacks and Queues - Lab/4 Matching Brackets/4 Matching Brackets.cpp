// 4 Matching Brackets.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
	...
	std::stack<std::string> expresionStack = std::stack<std::string>();
	std::string output = std::string();

	for (int i = 0; i < input.length(); i++)
	{
		expresionStack.push(std::string(1, input[i]));

		if (input[i] == ')')
		{
			std::string currentExpresion = myString().Empty();

			bool isLoopExit = false;

			while (!isLoopExit)
			{
				currentExpresion.insert(0, expresionStack.top());

				if (expresionStack.top() == "(")
				{
					isLoopExit = true;
				}
			...
			expresionStack.push(currentExpresion);

			output.append(currentExpresion).append(myString().NewLine());
		}
	}
