// 02 Remove Negatives and Reverse.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
	...
	std::vector<int> reversedList = std::vector<int>();

	for (int currentNum : list)
	{		
		if (currentNum >= 0)
		{
			reversedList.insert(reversedList.begin(), currentNum);
		}
	}
	...
	{
		output.append(input.Join(" ", &reversedList));
	}
