...
public:
	...
	std::vector<int>* ConvertStringToIntVector()
	{
		...
		return this->ConvertStringToIntVector(&result);
	}
...
int main()
{
	const myString STOP = "stop";

	std::vector<std::string> orderList = std::vector<std::string>();
	std::map<std::string, int> nums = std::map<std::string, int>();
	...
	for (std::string res : orderList)
	{
		output.append(res).append(" -> ").append(std::to_string(nums[res])).append(myString().NewLine());
	}
